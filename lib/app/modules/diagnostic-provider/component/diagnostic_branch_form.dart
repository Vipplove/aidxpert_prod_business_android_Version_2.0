...
import '../diagnostics_provider_dashboard/controllers/diagnostics_provider_dashboard_controller.dart';

class DiagnosticBranchFormModal extends StatefulWidget {
  final Map<String, dynamic>? branchData;
  const DiagnosticBranchFormModal(this.branchData, {super.key});

  @override
  State<DiagnosticBranchFormModal> createState() =>
      _DiagnosticBranchFormModalState();
}

class _DiagnosticBranchFormModalState extends State<DiagnosticBranchFormModal> {
  ...
  // Determine role based on context (you can pass this if needed)
  String get _roleFilter => 'Radiologist'; // Change to 'Pathologist' for Labs
  String get _prefix => _roleFilter == 'Pathologist' ? 'PATH' : 'RAD';

  @override
  void initState() {
    super.initState();
    print(widget.branchData);
    searchCtrl = TextEditingController(
      text: 'RAD${widget.branchData?['user']['user_id'] ?? ''} - '
          '${widget.branchData?['user']['first_name'] ?? ''} '
          '${widget.branchData?['user']['last_name'] ?? ''}',
    );
    nameCtrl =
        TextEditingController(text: widget.branchData?['center_name'] ?? '');
    descCtrl = TextEditingController(
        text: widget.branchData?['center_description'] ?? '');
    addressCtrl =
        TextEditingController(text: widget.branchData?['center_address'] ?? '');
    ...
    final photos = widget.branchData?['center_photos'] as List<dynamic>? ?? [];
  ...
  // Fixed: Safe prefix generation
  String _safePrefix(int userId) {
    final code = '$_prefix$userId';
    return code.length > 8 ? code.substring(0, 8) : code; // Safe limit
  }

  // API Call - Filter by role
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title:
            Text(widget.branchData == null ? 'Add New Center' : 'Edit Center'),
        centerTitle: true,
        actions: [
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // TYPEAHEAD SEARCH
            _sectionTitle('Assign $_roleFilter (Optional)'),
            TypeAheadField<Map<String, dynamic>>(
              textFieldConfiguration: TextFieldConfiguration(
                controller: searchCtrl,
                decoration: InputDecoration(
                  hintText: 'Search $_roleFilter by name or email...',
              ...
              onSuggestionSelected: (user) {
                ...
                final name =
                    'RAD${user['user_id'] ?? ''} - ${user['first_name'] ?? ''} ${user['last_name'] ?? ''}'
                        .trim();
                searchCtrl.text = name;
                customToast(
                    '$_roleFilter selected: ${_safePrefix(user['user_id'])}',
                    Colors.green);
              },
              noItemsFoundBuilder: (_) => Padding(
                padding: const EdgeInsets.all(16),
                child: Text('No $_roleFilter found'),
              ),
            ...
            _buildField(nameCtrl, 'Center Name', Icons.local_hospital,
                maxLines: 2),
        ...
        return Stack(
          children: [
            ClipRRect(
              borderRadius: BorderRadius.circular(12),
              child: isNet
                  ? (item['path'] != null && (item['path'] as String).isNotEmpty
                      ? CachedNetworkImage(
  ...
  Future<void> _save() async {
    if (nameCtrl.text.trim().isEmpty || addressCtrl.text.trim().isEmpty) {
      customToast('Center name and address are required', Colors.red);
      return;
    }

    final payload = {
      'center_name': nameCtrl.text.trim(),
      'center_description': descCtrl.text.trim(),
      'center_address': addressCtrl.text.trim(),
    ...
    // ---- FIXED FOR UPDATE MODE ----
    if (widget.branchData != null &&
        widget.branchData!['diagnostic_center_id'] != null) {
      payload['diagnostic_center_id'] =
          widget.branchData!['diagnostic_center_id'];
    }
